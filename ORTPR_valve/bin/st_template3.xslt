...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="stateMachine">
        ...
        <xsl:apply-templates mode="FUNCTION_DEFINITIONS" select="//actionNode[generate-id(.) = generate-id(key('distinctOutputActions', parameter[@name='name']/@value)[ancestor::stateMachine/@name=$stateMachineName])]">
            <xsl:sort select="@type"/>
            <xsl:sort select="@name"/>
        </xsl:apply-templates>
        <xsl:apply-templates mode="FUNCTION_DEFINITIONS" select="//actionNode[generate-id(.) = generate-id(key('distinctAutomataCalls', parameter[@name='automata']/@value)[ancestor::stateMachine/@name=$stateMachineName])]">
            <xsl:sort select="@type"/>
            <xsl:sort select="@name"/>
        </xsl:apply-templates>
        <xsl:apply-templates mode="FUNCTION_DEFINITIONS" select="//conditionNode[generate-id(.) = generate-id(key('distinctInputVariables', parameter[@name='name']/@value)[ancestor::stateMachine/@name=$stateMachineName])]">
            <xsl:sort select="@type"/>
            <xsl:sort select="@name"/>
        </xsl:apply-templates>
    ...
    <xsl:template match="conditionNode[@type='EVENT']" mode="SWITCH_BLOCK">
        <xsl:text>InParam.</xsl:text><xsl:value-of select="@name"/>
    </xsl:template>
