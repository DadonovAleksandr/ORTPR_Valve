...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/model">
        ...
        <xsl:text>(* Для правки нужно использовать исходник *.vsd и программу генерации (запустите скрипт build_all.bat). *)&#x0A;</xsl:text>
        <xsl:text>(* Автор шаблона генератора: Кернер А.В. 2018 год. *)&#x0A;</xsl:text>
    ...
    <xsl:template match="conditionNode[@type='EVENT']" mode="SWITCH_BLOCK">
        <xsl:text>DataIn.</xsl:text><xsl:value-of select="@name"/>
    </xsl:template>
